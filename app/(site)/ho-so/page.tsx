...
const messages: Record<string, { ok?: boolean; text: string }> = {
  profile: { ok: true, text: "Đã cập nhật hồ sơ." },
  password: { ok: true, text: "Đổi mật khẩu thành công." },
  name: { text: "Tên không được để trống." },
  missing: { text: "Vui lòng điền đầy đủ thông tin." },
  short: { text: "Mật khẩu mới tối thiểu 6 ký tự." },
  match: { text: "Mật khẩu nhập lại không khớp." },
  current: { text: "Mật khẩu hiện tại không đúng." },
};
...
export default async function ProfilePage({
  ...
  const flash = ok ? messages[ok] : error ? messages[error] : null;
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted">Quản lý thông tin & bảo mật tài khoản</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <Stat icon={Wallet} label="Số dư" value={`$${user.balance.toFixed(2)}`} />
        <Stat icon={ShieldCheck} label="Vai trò" value={user.role === "admin" ? "Quản trị" : "Khách hàng"} />
        <Stat
          icon={CalendarDays}
          label="Tham gia"
          value={new Intl.DateTimeFormat("vi-VN").format(user.createdAt)}
        />
        <Stat icon={User} label="Trạng thái" value="Đang hoạt động" />
      </div>

      {/* Edit profile */}
      <section className="rounded-2xl glass p-6">
        <h2 className="mb-4 flex items-center gap-2 font-display text-lg font-bold text-parchment">
          <User className="h-5 w-5 text-gold-400" /> Thông tin cá nhân
        </h2>
        <form action={updateProfileAction} className="space-y-4">
          ...
          <div>
            <label className={labelCls}>Email (không thể đổi)</label>
      ...
      <section className="rounded-2xl glass p-6">
        <h2 className="mb-4 flex items-center gap-2 font-display text-lg font-bold text-parchment">
          <KeyRound className="h-5 w-5 text-gold-400" /> Đổi mật khẩu
        </h2>
        <form action={changePasswordAction} className="space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <input name="next" type="password" required className={inputCls} placeholder="Tối thiểu 6 ký tự" />
            </div>
            <div>
